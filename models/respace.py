...
from .gaussian_diffusion import GaussianDiffusion, GaussianDiffusionDDPM, MaskedGaussianDiffusion
...
class MaskedSpacedDiffusion(MaskedGaussianDiffusion):
    """
    A diffusion process which can skip steps in a base diffusion process.
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = MaskedGaussianDiffusion(**kwargs)  # pylint: disable=missing-kwoa
        new_sqrt_etas = []
        for ii, etas_current in enumerate(base_diffusion.sqrt_etas):
            if ii in self.use_timesteps:
                new_sqrt_etas.append(etas_current)
