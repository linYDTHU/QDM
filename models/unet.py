...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        base_chn =  in_channels if cond_lq else 0
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        if cond_lq and lq_size == image_size:
            ...
            base_chn =  1 + in_channels if cond_mask else in_channels
        elif cond_lq and lq_size > image_size:
            feature_extractor = []
            feature_chn = 1 + in_channels if cond_mask else in_channels
            base_chn = 16
...
def meausure_memory_time_unet_swin():
    ...
    print(f"Total number of parameters in UNetModel: {total_params/1000**2:.2f}M")

    # Create a dummy input for testing
    batch_size = 1
    dummy_input = th.randn(batch_size, 3, 1024, 1024, device="cuda")
    lq = th.randn(batch_size, 3, 1024, 1024, device="cuda")
    timesteps = th.randint(0, 1000, (batch_size,), device="cuda")

    # Track initial memory usage
    th.cuda.reset_max_memory_allocated()
    initial_memory = th.cuda.memory_allocated(device="cuda")

    # Run a forward pass to test memory consumption
    with th.no_grad():  # Use no_grad to avoid storing gradients
        _ = model(dummy_input, timesteps, lq=lq)

    # Check memory usage after forward pass
    max_memory = th.cuda.max_memory_allocated(device="cuda")

    print(f"Max Memory Allocated During Forward Pass: {(max_memory-initial_memory) / (1024 ** 3):.2f} GB")

    num_warmup_runs = 1  # Number of warm-up runs to stabilize the model
    ...
    with th.no_grad():
        for _ in range(num_measurement_runs):
            ...
            _ = model(dummy_input, timesteps, lq=lq)
            end_event.record()
            th.cuda.synchronize()  # Ensure that all operations are completed
...
def meausure_memory_time_unet():
    ...
    num_warmup_runs = 1  # Number of warm-up runs to stabilize the model
    num_measurement_runs = 3  # Number of runs to average over for accurate timing
    # Warm-up on GPU
    with th.no_grad():
        for _ in range(num_warmup_runs):
            _ = model(dummy_input, timesteps, lq=lq)
...
if __name__ == "__main__":
    # meausure_memory_time_unet_swin()
    meausure_memory_time_unet()