...
class GaussianDiffusion:
    ...
    def decode_first_stage(self, z_sample, first_stage_model=None, consistencydecoder=None):
        ...
        if first_stage_model is None:
            pass
        elif consistencydecoder is None:
...
class MaskedGaussianDiffusion(GaussianDiffusion):
    """
    Utilities for training and sampling diffusion models with sparse masks.
    ...
    :param sf: super resolution factor
    """
    def q_mean_variance(self, x_start, y, mask, t):
        """
        Get the distribution q(x_t | x_0).

        :param x_start: the [N x C x ...] tensor of noiseless inputs.
        :param y: the [N x C x ...] tensor of degraded inputs.
        :param mask: Masks where 'True's represent areas to be predicted
        :param t: the number of diffusion steps (minus 1). Here, 0 means one step.
        :return: A tuple (mean, variance, log_variance), all of x_start's shape.
        """
        mean = _extract_into_tensor(self.etas, t, x_start.shape) * (y - x_start) + x_start
        variance = _extract_into_tensor(self.etas, t, x_start.shape) * self.kappa**2 * mask + 1e-7 * (1-mask)
        log_variance = variance.log()
        return mean, variance, log_variance

    def q_sample(self, x_start, y, mask, t, noise=None):
        """
        Diffuse the data for a given number of diffusion steps.
        ...
        return (
            _extract_into_tensor(self.etas, t, x_start.shape) * (y - x_start) + x_start
            + (_extract_into_tensor(self.sqrt_etas * self.kappa, t, x_start.shape) * mask + 1e-7 * (1 - mask)) * noise
        )

    def q_posterior_mean_variance(self, x_start, x_t, mask, t):
        """
        Compute the mean and variance of the diffusion posterior:
        ...
        posterior_variance = _extract_into_tensor(self.posterior_variance, t, x_t.shape) * mask + 1e-7 * (1-mask)
        posterior_log_variance_clipped = _extract_into_tensor(
            self.posterior_log_variance_clipped, t, x_t.shape
        ) * mask - 20 * (1-mask)
        assert (
    ...
    def p_mean_variance(
        ...
        model_variance = _extract_into_tensor(self.posterior_variance, t, x_t.shape) * mask + 1e-7 * (1-mask)   
        model_log_variance = _extract_into_tensor(self.posterior_log_variance_clipped, t, x_t.shape) * mask - 20 * (1-mask)
        ...
        if self.model_mean_type == ModelMeanType.START_X:      # predict x_0
            pred_xstart = process_xstart(model_output)
        elif self.model_mean_type == ModelMeanType.RESIDUAL:      # predict y-x
            pred_xstart = process_xstart(
                self._predict_xstart_from_residual(y=y, residual=model_output)
                )                              
        else:
        ...
        model_mean, _, _ = self.q_posterior_mean_variance(
            x_start=pred_xstart, x_t=x_t, mask=mask, t=t
        )
    ...
    def p_sample(self, model, x, y, mask, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, noise_repeat=False):
        """
        Sample x_{t-1} from the model at the given timestep.
        ...
        sample = out["mean"] + nonzero_mask * (th.exp(0.5 * out["log_variance"]) * mask + 1e-7 * (1-mask)) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "mean":out["mean"]}
    ...
    def p_sample_loop_progressive(
        ...
        if noise_repeat:
            noise = noise[0,].repeat(z_y.shape[0], 1, 1, 1)
        z_sample = self.prior_sample(z_y, mask, noise)
    ...
    def prior_sample(self, y, mask, noise=None):
        """
        Generate samples from the prior distribution, i.e., q(x_T|x_0) ~= N(x_T|y, ~)
        ...
        return y + (_extract_into_tensor(self.kappa * self.sqrt_etas, t, y.shape) * mask + 1e-7 * (1 - mask)) * noise

    def training_losses(
            self, model, x_start, y, mask, t,
            first_stage_model=None,
            model_kwargs=None,
            noise=None,
            ):
        ...
        z_t = self.q_sample(z_start, z_y, mask, t, noise=noise)

        terms = {}

        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            if "up_pred" in model_kwargs.keys():
                up_pred = model_kwargs["up_pred"]
                if up_pred:
                    model_output, pred = model(self._scale_input(z_t, t), t, **model_kwargs)
                else:
                    model_output = model(self._scale_input(z_t, t), t, **model_kwargs)
            else:
                up_pred = False
            ...
            assert model_output.shape == target.shape == z_start.shape
            if up_pred:
                terms["mse_up"] = mean_flat((z_start - pred) ** 2)
            terms["mse"] = mean_flat((target - model_output) ** 2)
        else:
            raise NotImplementedError(self.loss_type)
        ...
        if up_pred:
            return terms, z_t, pred_zstart, pred
        else:
            return terms, z_t, pred_zstart