...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        ...
        n_dim = _tensor.dim()
        if n_dim == 4:
            img_np = vutils.make_grid(_tensor, nrow=int(math.sqrt(_tensor.size(0))), normalize=False).numpy()
...
def center_crop(im, pch_size):
    '''
    Center crop a patch from the give image.
    '''
    h, w = im.shape[:2]
    # padding if necessary
    if h < pch_size or w < pch_size:
...
class CenterCrop:
    ...
    def __call__(self, im):
        ...
        if isinstance(im, list) or isinstance(im, tuple):
            out = []
            for current_im in im:
                out.append(center_crop(current_im, self.pch_size))
        else:
            out = center_crop(im, self.pch_size)
        return out
...
# ----------------------quadtree_mask----------------------------
@torch.no_grad()
def generate_quadtree_masks(I, s, H_m, W_m):
    """
    Efficient generation of binary quadtree masks using max pooling.

    Parameters:
    - I (torch.Tensor): A batch of low-quality images of shape (B, C, H, W).
    - s (float): Threshold value.
    - H_m (int): Desired mask height.
    - W_m (int): Desired mask width.

    Returns:
    - M (torch.Tensor): Binary quadtree masks of shape (B, H_m, W_m) with values {0, 1}.
    """

    # Step 3: Determine the scale level `l`
    _, _, H, W = I.shape
    l = max(int(np.log2(H / H_m)), int(np.log2(W / W_m)))
    l = max(l, 1)  # Ensure l is at least 1

    # Step 5: Max pooling on I with kernel size 2^l x 2^l to obtain V^+
    kernel_size = 2 ** l
    V_plus = F.max_pool2d(I, kernel_size=kernel_size)

    # Step 6: Max pooling on -I with kernel size 2^l x 2^l to obtain V^-
    V_minus = F.max_pool2d(-I, kernel_size=kernel_size)

    # Step 7: Generate mask M based on threshold `s`
    M = ((V_plus + V_minus).mean(dim=1) >= s).float()

    # Step 8: Resize M to desired mask dimensions (H_m, W_m) using nearest-neighbor interpolation
    M = F.interpolate(M.unsqueeze(1), size=(H_m, W_m), mode='nearest').squeeze(1)

    # Step 9: Return the binary mask M
    return M
